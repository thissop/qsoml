...
def custom_loss(y_true, y_pred, latent_vectors, latent_augmented, rest_spectra):
    fid_loss = tf.reduce_mean(tf.square(y_true - y_pred))
    w_prime = tf.ones_like(rest_spectra, dtype=tf.float32)
...
@tf.function
def train_step(y_batch, z_batch, z_aug_batch):
    with tf.GradientTape() as tape:
        ...
        loss = custom_loss(y_batch, y_pred, latent_vectors, latent_augmented, rest_spectra)
    gradients = tape.gradient(loss, autoencoder.trainable_variables)
    optimizer.apply_gradients(zip(gradients, autoencoder.trainable_variables))
    return loss

@tf.function
def val_step(y_batch, z_batch, z_aug_batch):
...
for epoch in range(num_epochs):
    ...
    for i in range(0, len(y_train), batch_size):
        ...
        loss = train_step(y_batch, z_batch, z_aug_batch)
    ...
    for i in range(0, len(y_test), batch_size):
        ...
        z_aug_batch = tf.convert_to_tensor(z_aug_all[epoch, i:i+batch_size], dtype=tf.float32)
        val_loss = val_step(y_batch, z_batch, z_aug_batch)
